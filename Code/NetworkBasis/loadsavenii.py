...
def save_disp(data,path,filename, use_orig_file_info=False, do_resample=True, i=0):
    ...
    if zoom_factor[2] == 4/3: #only needed for one image in the M2olie dataset: zoom function round down when non-integer shape, so use 1.34 instead of 1.333333
        zoom_factor = (zoom_factor[0], zoom_factor[1], 1.35)
